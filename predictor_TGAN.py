...
def run_external_split(i: int, X_full: pd.DataFrame, y_full: pd.Series, k: int) -> dict:
    ...
    train_table = pd.concat(
        [X_trainval.reset_index(drop=True),
         y_trainval.reset_index(drop=True)], axis=1
    )

    # 2. Genera files sintètiques (~50 % de les reals)
    n_synth = int(len(X_trainval))
    synth_table = tgan_sample(train_table, n_synth)

    # 3. Separa de nou X i y i (opcional) aplica l’escalador ja entrenat
    X_synth = pd.DataFrame(
        scaler.transform(synth_table[X_full.columns]),
        columns=X_full.columns
    )
    y_synth = synth_table[y_trainval.name]

    # 4. Concatena real + sintètic
    X_trainval_augmented = pd.concat([X_trainval, X_synth], ignore_index=True)
    y_trainval_augmented = pd.concat([y_trainval, y_synth], ignore_index=True)
