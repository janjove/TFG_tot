...
def run_external_split(i: int, X_full: pd.DataFrame, y_full: pd.Series, k: int) -> dict:
    ...
    n_bootstrap = int(len(X_trainval))

    # Juntem X i y per fer bootstrap mantenint la relació
    ...
    # Separem X i y del bootstrap
    X_trainval_bootstrap = Xy_bootstrap[X_full.columns]
    y_trainval_bootstrap = Xy_bootstrap[y_trainval.name]

    # Afegim soroll discret només a les característiques (no a y!)
    X_trainval_bootstrap_noisy = afegir_soroll_discret(X_trainval_bootstrap, prob_mutacio=0.1)

    # Concatenem l’original amb el bootstrap amb soroll
    X_trainval_augmented = pd.concat([X_trainval, X_trainval_bootstrap_noisy], ignore_index=True)
    y_trainval_augmented = pd.concat([y_trainval, y_trainval_bootstrap], ignore_index=True)
...
def main(path_sav: str = "CEJFEAjut2015Updated.sav", path_vars: str = "variables3.csv") -> None:
    ...
    # --- Guarda resultats
    outdir = pathlib.Path("outputs_overfit_bootstrap_2")
